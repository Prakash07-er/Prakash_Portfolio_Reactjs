import React from 'react'
import '../styles/resume.css'

export default function Resume() {
    return (
        <>
        <section id="resume" class="resume" data-aos="flip-left" data-aos-easing="ease-out-cubic" data-aos-duration="2000">
        <h1 class="section-heading-title">MY JOURNEY </h1>
      <div class="container">

        <div class="row">
          <div class="col-lg-6">
            <h3 class="resume-title">About Me</h3>
            <div class="resume-item pb-0">
              <h4>Prakash Umaganthan</h4>
              <p><em>Well-qualified Full Stack Developer familiar with wide range of programming utilities and languages. Born To Explore, Tries to learn something new every day,  digital/print marketing material from initial concept to final, polished deliverable.</em></p>
              <p>
                <h5>2021 - current</h5>
              <ul>
                <li>(+91) 77088 74955 </li>
                <li>er.u.prakashmahi@gmail.com</li>
              </ul>
              </p>
            </div>
              <div class="resume-item">
              <h4>proficiency</h4>
              <ul>
                <li>Well known with JavaScript</li>
                <li> MERN stack development </li>
                <li> WEB development (Wordpress, PHP & Webflow)</li>
                <li> Page developement with technical SEO perspective. </li>
              </ul>
            </div>
            <h3 class="resume-title">Education</h3>
            <div class="resume-item">
              <h4>Bachelor of Technology - ECE</h4>
              <h5>2014 - 2018</h5>
              <p><em>Christ college of engineering and  Technology, Pondicherry, IN</em></p>
              <p>Completed my graduation in Electronics and Commucnication stream without any career gap and gained 8CGPA </p>
            </div>
           
          </div>
          <div class="col-lg-6">
            <h3 class="resume-title">Professional Experience</h3>
            <div class="resume-item">
              <h4>APP DEVELOPER (Full stack & Web)</h4>
              <h5>Aug-2021 to Jan-2023</h5>
              <p><em>Pipecandy Technologies Chennai, IN </em></p>
              <p>
              <ul>
                <li>Developed Fullstack & frontend application with full responsiveness for learning learning</li>
                <li>Contributed towards frontend and backend development with reusable components(React.js, Node.js,express.js, Loopback, HTML/CSS,
                Meterialize).</li>
                <li>Ensuring the page development with respect to technical SEO metrics with core web vitals.</li>
                <li>Contributed for building website pages using wordpress with PHP stack as per the product
                requirement (PHP, postgress, wordpress shortcodes).</li>
                <li>Kindly refer Resume for more my responsibilities...</li>
              </ul>
              </p>
            </div>
            <div class="resume-item">
              <h4>Chat Associate - Amazon kindle support</h4>
              <h5>2018 - 2020</h5>
              <p><em>Sutherland Global pvt limited, Chennai, IN </em></p>
              <p>
              <ul>
                <li> Handilng the technical queries and non-technical queries of Amazon Kindle and Echo devices</li>
                <li>Managing the team target as well as individual target of team members. </li>
                <li>Manage large amounts of inbound and outbound calls in a timely manner</li>
              </ul>
              </p>
            </div>
            <div class="resume-item">
              <h4>Business process management Executive</h4>
              <h5>2020 - 2021</h5>
              <p><em>Conneqt Business solution, Chennai, IN</em></p>
              <p>
              <ul>
                <li>Identify customers' needs, clarify information, research every issue and provide solutions and/or alternatives..</li>
                <li>Build sustainable relationships and engage customers by taking the extra mile</li>
              </ul>
              </p>
            </div>
          </div>
        </div>

      </div>
    </section>
    </>
    )
}
